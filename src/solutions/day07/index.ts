  ...
  parse(input: string[]) {
    ...
    return input.map((i) => {
      ...
      return {
        ...
        v,
        order: parseInt(
          `${this.hands.length - this.hands.indexOf(v.type)}${this.getHandOrder(
            hand,
            this.cards
          )}`
        ),
    ...
    return input.map((i) => {
      ...
      if (hand.includes("J")) {
        ...
        const a = possibleHands.map(
          (p) =>
            this.hands.length -
            this.hands.indexOf(checks.find((c) => c(p) !== null)!(p)?.type!)
        );
        let maxHand = 0;
        ...
        return {
          hand,
          points: parseInt(points),
          order: parseInt(`${maxHand}${this.getHandOrder(hand, this.cards2)}`),
        };
      } else {
        const v = checks.find((c) => c(hand) !== null)!(hand)!;
